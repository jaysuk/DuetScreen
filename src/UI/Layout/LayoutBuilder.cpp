...
namespace UI::Layout
{
	...
	LvObj* LayoutBuilder::buildNode(
		const nlohmann::json& node, LvObj& parent, const std::string& path, LayoutInstance& instance, const WidgetRegistry& registry, bool isRoot)
	{
		const std::string id = nodeId(node, path);
			...
			if (node.contains("widget"))
			{
				...
				if (isRoot)
				{
					raw->setSize(LV_PCT(100), LV_PCT(100));
				}
		...
		const nlohmann::json& doc, LvObj& parent, const WidgetRegistry& registry, std::vector<std::string>* errorsOut)
	{
		...
		instance->m_root = buildNode(doc["root"], parent, "root", *instance, registry, /* isRoot */ true);
		return instance;
	}
